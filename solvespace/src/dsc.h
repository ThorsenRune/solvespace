...
//RT new 3d point element intended for scaling purposes etc.
class Double3D{
public:
    double x, y, z;
    void setSame(double CommonValue) {
        x=CommonValue;
        y=CommonValue;
        z=CommonValue;
    }
...
public:
    ...
    Vector ScaledBy3D(Double3D value3d);        //RT new 3D scaling
...
//RT start
    T *getByUId(uint32_t v) {	// Get element by UId number
        int first = 0, last = n-1;
        while(first <= last) {
            int mid = (first + last)/2;
            H hm = elem[mid].h;
...
//RT end
    ...
    void MoveSelfInto(IdList<T,H> *l) {
        ...
        elemsAllocated = n = 0;
        elem = NULL;			//The copy (l->elem) still points to the memory
    }
